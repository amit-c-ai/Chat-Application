...
class Client:
    ...
    def start(self):
        ...
        receive = Receive(self.client, self.name)

        # Start send and receive threads
        send.start()
        receive.start()

        self.client.sendall('{} has joined the server!'.format(self.name).encode('ascii'))
    ...
    def send(self, text_input):
        ...
        # Type 'QUIT' to leave the chatroom
        if message == 'QUIT':
            self.client.sendall('{} has left the server!'.format(self.name).encode('ascii'))
        ...
        # Send message to server for broadcasting
            ...
            self.client.sendall('{} : {}'.format(self.name, message).encode('ascii'))
